module.exports = function(commands, settings) {
  function formatCommand(command) {
    return {
      ...
      options: command.builder
        ? Object.keys(command.builder).map(function(key) {
    ...
    return [
      {
        ...
        text: command.desc,
        fields: info.options
          .concat([{ spec: '--help, -h, Prints this information' }])
          .map(option => {
            return {
              title: option.spec,
              value:
                option.info && Object.keys(option.info).length > 0
                  ? JSON.stringify(option.info)
                  : '',
              short: false
            };
          })
