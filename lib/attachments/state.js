module.exports = function(holder, settings) {
  ...
  let modes = Object.keys(holder)
    .filter(key => holder[key] === true)
    .map(key => key);
  ...
  let fallback =
    holder.tlTrack && holder.tlTrack.track.artists
      ? `${holder.tlTrack.track.artists[0].name}: ${holder.tlTrack.track.name}`
      : track;
