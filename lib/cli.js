...
module.exports = function() {
  ...
  const logger = createLogger({
    level: Object.keys(config.npm.levels).find(function(level) {
      return (
        config.npm.levels[level] ===
        Math.min(argv.verbose + 2, Object.keys(config.npm.levels).length - 1)
      );
    }),
      ...
      formatTrack: function(track) {
        return {
          ...
          artists: (track.artists || [])
            .map(artist => `<${artist.uri}|${artist.name}>`)
            .join(', '),
          album: track.album ? `<${track.album.uri}|${track.album.name}>` : '',
        };
      }
    }),
    params: {
      username: argv['display-name'],
