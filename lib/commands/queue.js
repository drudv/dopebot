module.exports = {
  ...
  handler: function(argv) {
        ...
        for (let i = 0; i < argv.db.search.results[type].items.length; i++) {
          if (argv.db.search.results[type].items[i].hit === argv.id) {
            ...
            if (argv.db.search.results[type].items[i].type === 'playlist') {
              if (
                !argv.uri.match(/spotify:user:[\w]+:playlist:[a-zA-Z0-9]{22}/)
              ) {
                argv.uri = argv.uri.replace(
                  /^spotify/,
                  argv.db.search.results[type].items[i].owner.uri
                );
                argv.db.logger.info(
                  'Fixing playlist uri by adding owner: ' + argv.uri
                );
              }
            }
          }
    ...
    if (
      ...
      !argv.uri.match(/spotify:[\w.-_:]+[a-zA-Z0-9]{22}/)
    ) {
      if (argv.uri.match(/^https?\:\/\//)) {
        argv.db.logger.info('Enqueueing external URL: ' + argv.uri);
        enqueueTracks(argv);
        return;
      }
    ...
    const matches = argv.uri.match(
      /spotify:user:(\w+):playlist:([a-zA-Z0-9]{22})/
    );
    ...
    if (matches) {
      ...
      argv.db.logger.info(
        "Adding playlist of user '" + user + "': " + playlist
      );
...
function enqueueTracks(argv) {
  argv.db.mopidy.tracklist
    .add([null, parseInt(argv.position), argv.uri, argv.uris])
...
function handlePlaylist(argv, user, playlist) {
  if (!argv.db.search.expires || argv.db.search.expires <= new Date()) {
    ...
    argv.db.spotify.clientCredentialsGrant().then(
      function(data) {
        argv.db.logger.info(
          'The access token expires in ' + data.body['expires_in']
        );
        argv.db.search.expires = new Date();
        argv.db.search.expires.setTime(
          argv.db.search.expires.getTime() + data.body['expires_in'] * 1000
        );
...
function lookupPlaylist(argv, user, playlist, offset) {
  argv.db.logger.info(
    'Looking up playlist tracks ' + (offset + 1) + '-' + (offset + 100)
  );

  argv.db.spotify
    .getPlaylistTracks(user, playlist, {
      offset: offset,
      limit: 100,
      fields: 'items,total'
    })
    .then(
      function(data) {
        ...
        if (argv.uris.length < data.body.total) {
          ...
          argv.db.logger.info(
            'Adding a total of ' + argv.uris.length + ' tracks to the tracklist'
          );
          enqueueTracks(argv);
        }
      },
      function(err) {
        argv.db.logger.error(
          'Something went wrong when loading the playlist tracks',
          err
        );
      }
    );
