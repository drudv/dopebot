...
function search(argv) {
  ...
  argv.db.spotify
    ...
    .then(function(data) {
      ...
      while (maxCount > 0 && typesCount > 0) {
        for (let t = 0; t < typesCount; t++) {
          ...
          if (data.body[type].items[length]) {
            argv.db.search.results[type].items[length] =
              data.body[type].items[length];
            maxCount--;
          } else {
            types.splice(t, 1);
            typesCount--;
      ...
      if (
        argv.db.search.results.albums &&
        argv.db.search.results.albums.items.length > 0
      ) {
        return argv.db.spotify.getAlbums(
          argv.db.search.results.albums.items.map(function(album) {
            return album.id;
          })
        );
      } else {
    ...
    .then(function(data) {
      if (data && data.body) {
        for (let i = 0; i < data.body.albums.length; i++) {
          Object.assign(
            argv.db.search.results.albums.items[i],
            data.body.albums[i]
          );
        }
      }
