module.exports = {
  ...
  handler: function(argv) {
    if (
      argv.db.state.random ||
      (argv.db.state.repeat && argv.db.state.single) ||
      argv.db.state.single
    ) {
      argv.db.logger.info('Getting next track');
      argv.db.mopidy.tracklist
        .nextTrack([argv.db.state.tlTrack])
    ...
    argv.db.mopidy.tracklist
      ...
      .then(function(value) {
        length = value;
        return argv.db.mopidy.tracklist.slice([
          index,
          index + parseInt(argv.length)
        ]);
      })
      .catch(function() {})
